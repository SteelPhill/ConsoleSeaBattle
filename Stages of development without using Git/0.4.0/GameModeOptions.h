...
void PlayerVsEasyAI(ShipPlacementVariantsMenuItems placingShipsChosenMethod)
{
	...
	do
	{
		...
		{
			AIShot.result.isKilled = true;
			AIShot = ComputerShooting(playerField, AIShot);
...
void EasyAIVsEasyAI()
{
	...
	do
	{
		do
		{
			...
			firstAIShot = ComputerShooting(secondAIField, firstAIShot);
		...
		do
		{
			...
			secondAIShot = ComputerShooting(firstAIField, secondAIShot);
