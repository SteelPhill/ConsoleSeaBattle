...
inline bool IsShipDefinitelyNotKilled(char field[Rows][Columns], const ShotResult& hit)
{
...
inline bool IsNoMoreHitsAround(char field[Rows][Columns], const ShotResult& hit)
{
...
inline bool IsHitHorizontalShip(char field[Rows][Columns], const ShotResult& hit)
{
	return field[hit.y][hit.x - 1] == Hit || field[hit.y][hit.x + 1] == Hit;
}

//ПРОВЕРКА НА ПОПАДАНИЕ В ВЕРТИКАЛЬНЫЙ КОРАБЛЬ
inline bool IsHitVerticalShip(char field[Rows][Columns], const ShotResult& hit)
{
	return field[hit.y - 1][hit.x] == Hit || field[hit.y + 1][hit.x] == Hit;
}

//ПРОВЕРКА НА УНИЧТОЖЕНИЕ ГОРИЗОНТАЛЬНОГО КОРАБЛЯ
inline bool IsHorizontalShipKilled(char field[Rows][Columns], const ShotResult& hit)
{
...
inline bool IsVerticalShipKilled(char field[Rows][Columns], const ShotResult& hit)
{
