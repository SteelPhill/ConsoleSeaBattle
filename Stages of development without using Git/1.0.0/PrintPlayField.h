...
void PrintShotCoordinates(const Cell& shot)
{
	...
	cout << (char)(symbolCodeLocatedBeforeSymbolA + shot.coordinates.abscissa) << "-" << shot.coordinates.ordinate - 1 << endl << endl;
}
...
{
	cout << "\t\t\t " << "Осталось уничтожить кораблей: " << lastShot.result.ShipsLeftAfterLastShot << endl << endl;
}

//ПЕЧАТЬ ИМЁН ИГРОКОВ	
void PrintPlayerNames(const string& firstPlayerName, const string& secondPlayerName, const string& shooterName)
{
