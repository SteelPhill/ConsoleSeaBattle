...
{
	return field[hit.coordinates.ordinate][hit.coordinates.abscissa - 1] == Hit || 
		field[hit.coordinates.ordinate][hit.coordinates.abscissa + 1] == Hit;
}

//ПРОВЕРКА НА ПОПАДАНИЕ В ВЕРТИКАЛЬНЫЙ КОРАБЛЬ
inline bool IsHitVerticalShip(char field[Rows][Columns], const Cell& hit)
{
	return field[hit.coordinates.ordinate - 1][hit.coordinates.abscissa] == Hit ||
		field[hit.coordinates.ordinate + 1][hit.coordinates.abscissa] == Hit;
}

//ПРОВЕРКА НА УНИЧТОЖЕНИЕ ГОРИЗОНТАЛЬНОГО КОРАБЛЯ
bool IsHorizontalShipKilled(char field[Rows][Columns], const Cell& hit)
{
	for (int i = hit.coordinates.abscissa - 1; field[hit.coordinates.ordinate][i] != Empty &&
		field[hit.coordinates.ordinate][i] != Miss; i--)
	{
...
{
	for (int i = hit.coordinates.ordinate - 1; field[i][hit.coordinates.abscissa] != Empty &&
		field[i][hit.coordinates.abscissa] != Miss; i--)
	{
