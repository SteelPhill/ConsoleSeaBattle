	...
	PlayerData& secondPlayerData)
{
	...
	if (isNewGame)
	{
		if (firstPlayerData.isPlayer)
		{
			...
			PrintBattlefield(difficulty, secondPlayerField, firstPlayerField, 
				firstPlayerShot, noOneShot, secondPlayerData, firstPlayerData);
		}
		else if (secondPlayerData.isPlayer)
		{
			...
			cout << "\t\t\t      " << "Стартовое поле" << endl << endl;
			PrintBattlefield(difficulty, firstPlayerField, secondPlayerField, 
				firstPlayerShot, noOneShot, firstPlayerData, secondPlayerData);
		}
		else
		{
			AutoFillField(firstPlayerField);
	...
	if (firstPlayerData.isMove)
	{
		do
		{
			isGameContinue = TryMove(gameMode, difficulty, secondPlayerField, 
				firstPlayerField, secondPlayerShot, firstPlayerShot, secondPlayerData, firstPlayerData);

			if (!isGameContinue)
				return;

			isGameContinue = TryMove(gameMode, difficulty, firstPlayerField, 
				secondPlayerField, firstPlayerShot, secondPlayerShot, firstPlayerData, secondPlayerData);
