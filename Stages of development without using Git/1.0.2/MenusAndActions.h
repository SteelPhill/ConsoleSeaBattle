...
void PrintRules()
{
	...
	cout << "7.Координаты назначаются следующим образом: первой вводится буква, затем цифра." << endl;
	...
	cout << " [O] - неподбитый корабль;" << endl;
	cout << " [*] - промах;" << endl;
...
//ВВОД ИМЕНИ ИГРОКА
void InputPlayerName(PlayerData& firstPlayerData)
{
	...
	PlayerData& secondPlayerData)
{
	...
	{
		secondPlayerData.name = "Компьютер";
	...
	PlayerData& secondPlayerData)
{
	...
	{
		InputPlayerName(firstPlayerData);

		secondPlayerData.isFirstAI = false;

		isProgramContinue = 
			TryContinueSelectedInDifficultyMenu(gameMode, difficulty, firstPlayerData, secondPlayerData);
	...
	PlayerData& secondPlayerData)
{
	...
	do
	{
		...
		{
			isProgramContinue = 
				TryContinueSelectedInModesMenu(gameMode, difficulty, firstPlayerData, secondPlayerData);
		...
		{
			bool isGameLoad = TryLoadGame(gameMode, difficulty, firstPlayerField, secondPlayerField,
				firstPlayerLastShot, secondPlayerLastShot, firstPlayerData, secondPlayerData);
	...
	PlayerData& shooterData)
{
	...
	do
	{
		...
		case PauseMenuItems::Save:
		{
			...
			if (targedData.isPlayer || targedData.isFirstAI)
				SaveGame(gameMode, difficulty, targedField, shooterField,
					targedShot, shooterShot, targedData, shooterData);
			else if (shooterData.isPlayer || shooterData.isFirstAI)
				SaveGame(gameMode, difficulty, shooterField, targedField,
					shooterShot, targedShot, shooterData, targedData);
	...
	if (shooterData.isPlayer || targedData.isFirstAI)
		PrintBattlefield(difficulty, targedField, shooterField,
			shooterShot, shooterData.name, targedData, shooterData);
	else if (targedData.isPlayer || shooterData.isFirstAI)
		PrintBattlefield(difficulty, shooterField, targedField,
