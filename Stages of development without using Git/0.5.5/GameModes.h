				...
				do
				{
					...
					case PauseMenuItems::Save:
					{
						if (targedData.isPlayer || shooterData.isFirstAI)
							SaveGame(gameMode, difficulty, shooterField, targedField, shooterShot, targedShot, shooterData.name, targedData.name);
						else if(targedData.isFirstAI || shooterData.isPlayer)
							SaveGame(gameMode, difficulty, targedField, shooterField, targedShot, shooterShot, targedData.name, shooterData.name);
						continue;
					}
	...
	if (isNewGame)
	{
		...
		if (firstPlayerData.isPlayer || secondPlayerData.isPlayer)
		{
			if (firstPlayerData.isPlayer)
			{
				...
				PrintBattlefield(secondPlayerField, firstPlayerField, firstPlayerShot, NoOneShot, firstPlayerData, secondPlayerData);
				system("pause");
				system("cls");
			}
