...
void InputCoordinates(char field[Rows][Columns], Cell& cell, const int shipSize)
{
	...
	if (shipSize == NULL)
		cout << "Введите координаты выстрела и нажмите Enter:" << endl;
	else
		cout << "Введите начальные координаты (первую клетку) корабля и нажмите Enter:" << endl;
	...
	do
	{
		cin >> cell.coordinates.ordinate;

		if (cell.coordinates.ordinate < beginRange || cell.coordinates.ordinate > endRange)
			cout << "Ошибка ввода!" << endl;

	} while (cell.coordinates.ordinate < beginRange || cell.coordinates.ordinate > endRange);
}